...
function tokenize(text: string, lang: string): string[] {
  ...
  if (lang === "en") {
    ...
    words = nodejieba.cut(text).filter((word: string) => !stopwords.has(word));
  }
  return words;
}
...
function splitTimeAndCountByMinutes(times: Date[]): {
  ...
  // 计算每个时间与最小时间的分钟差
  const minutesFromStart = timestamps.map((timestamp) =>
    Math.floor((timestamp - minTime) / 60000)
  );
  // 计算总的时间跨度，按分钟计算
...
parentPort!.on(
  ...
  async (message: {
    ...
    let speeches = speechesLangMay.values().next().value!;
    {
      ...
      // 转换为所需格式
      durationPieChartById = Object.entries(userSpeechCount).map(
        ([userId, count]) => ({
          value: count,
          userId: userId,
        })
    ...
    { 
      ...
      for (let lang of speechesLangMay.keys()){
        ...
        let summary=''
        try{
          summary=await aiClient.intelligentSummary(texts,lang as 'zh' | 'en')
        }catch(e){
          console.log(e)
        }
    ...
    parentPort!.postMessage({
      meetId,
      data: { wordCloud, chatHeatMap, durationPieChartById,intelligentSummary},
    });
  }
);
