...
class MeetSummaryUtil{
    ...
    initMeetSummaryService(){
        ...
        this.meetSummaryWorker.on('message',async ({meetId,data:{wordCloud,durationPieChartById,chatHeatMap}})=>{
            ...
            meetRoomRecordDao.updateSummary(meetId,wordCloud,durationPieChart,chatHeatMap)
        })
    ...
    async summary(meetId:string){
        ...
        for (const lang of langs) {
            let speeches=await meetSpeechDao.findSpeechesByRoomAndLang(meetId,lang)
            speechesLangMay.set(lang,speeches)
        }
        this.workerTaskQueue.addTaskToQueue({action:'summary',data:{meetId,speechesLangMay}})
    }
}
